...
{
    [Route("admin/roles")]
    public class RolesControllers : AdminControllerBase<RolesModels>
    {
        #region Methods
        [HttpGet]
        ...
        {
            var submitModel = values.DeserializeJsonTo<RolesModels.ViewModel.GridModel.GridItem>() ?? new RolesModels.ViewModel.GridModel.GridItem();
        ...
        {
            await Model.CRUD(databaseAction: Enums.DatabaseActions.DELETE, roleID: key, submitModel: new RolesModels.ViewModel.GridModel.GridItem());
            if (Model.Form.HasErrors)
            {
                return GetDevexpressErrorResult(Model.Form.ErrorMessage);
            }
